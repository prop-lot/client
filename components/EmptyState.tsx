...
const EmptyState = ({ appliedFilters, error, clearFilters }: { appliedFilters: string[], error: any, clearFilters: () => void }) => {
    ...
    <>
      <div className="text-center place-items-center grid">
        <Image
          ...
          height="120"
        />
        <h1 className="font-medium font-londrina text-3xl py-[8px]">No ideas found</h1>
        <p className="font-medium font-propLot pt-[8px] pb-[16px]">{message}</p>
        {appliedFilters.length > 0 && !error && (
          <button onClick={() => clearFilters()} className="bg-gray-700 text-white font-bold !border-none !text-[16px] flex-1 sm:flex-none !rounded-[10px] !font-propLot !pt-[8px] !pb-[8px] !pl-[16px] !pr-[16px]">
            Clear filters
          </button>
        )}
        {error && (
