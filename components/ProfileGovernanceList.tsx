...
const ProposalWrapper = ({
  ...
  const onClick = () => {
    type === 'LIL_NOUN' ? Router.push(`/vote/${id}`) : Router.push(`/vote/nounsdao/${id}`);
  };
    ...
    <>
      ...
      {/* {sortedProposals?.map(p => {
        ...
        if (p.type === 'BIG_NOUN') {
          ...
          const propStatus = undefined // bigNounsPropStatus(p.proposal, snapshotVoteObject);
          return (
            <div className="mb-[16px] space-y-4" key={p.proposal.id}>
              <ProposalWrapper
                id={p.proposal.id}
